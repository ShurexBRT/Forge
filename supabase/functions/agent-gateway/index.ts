  ...
  try {
    ...
    if (action === 'handoff') {
      ...
      if (latestLease) {
        ...
        if (latestLease.execution_id !== executionId || latestLease.agent_id !== agent.id || latestLease.agent_role !== agent.role) return json({ error: 'Handoff execution_id does not own the latest ticket lease.' }, 409)
