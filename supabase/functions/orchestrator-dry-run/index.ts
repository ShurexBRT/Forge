...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (ticketIds.length) {
      const [depResult, leaseResult] = await Promise.all([
    ...
    const now = Date.now()
    const activeLeaseByTicket = new Map<string, any>()
    const expiredLeaseByTicket = new Map<string, any>()
    ...
    return json({
      mode: 'dry-run',
      selected: evaluations.find((i) => i.eligible) ?? null,
      evaluations,
      policy: {
