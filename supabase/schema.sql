-- Forge v0.2 Supabase schema draft.
-- This file is intentionally not a migration yet: Forge does not have its own
-- Supabase project at the time of writing. Apply and verify it on the Forge
-- project first, run advisors/tests, then capture the verified schema as a migration.
--
-- v0.2 access model: one authenticated owner per project. Multi-user project
-- membership is deliberately deferred so we do not ship permissive RLS early.
...
create index if not exists agent_runs_ticket_id_created_at_idx on public.agent_runs(ticket_id, created_at desc);
create index if not exists ticket_activity_ticket_id_created_at_idx on public.ticket_activity(ticket_id, created_at desc);
...
create or replace function public.create_default_agent_runs()
returns trigger
language plpgsql
security invoker
set search_path = ''
as $$
begin
  insert into public.agent_runs(ticket_id, role, status)
  select new.id, role, 'pending'
  from unnest(array['Planner', 'Builder', 'Reviewer', 'QA', 'Browser', 'Release']) as role;
  return new;
end;
$$;
...
-- AFTER is intentional: ticket_activity and agent_runs have FKs to tickets.
create trigger tickets_activity_log
after insert or update on public.tickets
for each row execute function public.log_ticket_activity();

create trigger tickets_default_agent_runs
after insert on public.tickets
for each row execute function public.create_default_agent_runs();
